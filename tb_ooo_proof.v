...
module tb_ooo_proof;
    ...
    always @(posedge clk) begin
        ...
        begin : per_cycle_block
            ...
            else if (t_inst_raw != 32'b0) begin
                // Hiển thị BP prediction nếu là branch
                if (dut.fetch_is_branch_comb)
                    $display("  [FETCH]    PC=0x%08h  inst=0x%08h  (%s)  [BP: %s -> 0x%08h]",
                             t_pc_out, t_inst_raw, mnem(t_inst_raw),
                             dut.bp_predict_taken ? "TAKEN" : "NOT-TAKEN",
                             dut.bp_predict_target);
                else
            ...
            if (t_cmt_v) begin
                if (dut.u_rob.commit_store)
                    ...
                    // INT register (bao gồm FLE/FLT/FEQ result)
    ...
    // BP Statistics
    // =========================================================================
    integer bp_total;       // tổng số lệnh branch được predict
    integer bp_correct;     // số lần predict đúng (không flush)
    integer bp_mispredict;  // số lần misprediction (có flush)
    reg     was_predicting; // đang theo dõi prediction của 1 branch

    initial begin
        bp_total = 0; bp_correct = 0; bp_mispredict = 0;
        was_predicting = 0;
    end

    // Count mispredictions = số lần flush xảy ra
    always @(posedge clk) begin
        if (rst_n && t_flush) begin
            bp_mispredict <= bp_mispredict + 1;
        end
        // Count predictions: mỗi khi fetch branch instruction
        if (rst_n && !t_stall && !t_flush
            && t_inst_raw != 32'b0
            && (t_inst_raw[6:0] == 7'b1100011)  // BRANCH opcode
           ) begin
            bp_total <= bp_total + 1;
        end
    end

    // bp_correct = total - mispredict (approximate, counted at DONE)
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (idle_cnt >= 5) begin
                ...
                $display("    Branches fetched : %0d", bp_total);
                $display("    Mispredictions   : %0d", bp_mispredict);
                if (bp_total > 0)
                    $display("    Accuracy         : %0d%%",
                             (bp_total - bp_mispredict) * 100 / bp_total);
                $display("  Xem cac dong [FETCH] co [BP:] de kiem tra prediction.");
                $display("==============================================================");
                $finish;
            end
